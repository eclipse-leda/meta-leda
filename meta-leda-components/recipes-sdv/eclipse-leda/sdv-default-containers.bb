...
do_install:append() {
    ...
    install ${THISDIR}/kanto-containers/cloudconnector.json ${D}${KANTO_MANIFESTS_DIR}
    ...
    install -d ${D}${CLOUDCONNECTOR_CERTIFICATES_DIR}
}

PACKAGES = "${PN}"
#FILES:${PN} += "${KANTO_MANIFESTS_DIR}"
FILES:${PN} += "${KANTO_MANIFESTS_DIR}/cloudconnector.json"
...
FILES:${PN} += "${CLOUDCONNECTOR_CERTIFICATES_DIR}"
